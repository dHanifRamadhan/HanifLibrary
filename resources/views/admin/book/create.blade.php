...
@section('main')
    ...
    <div class="mx-16 flex">
        <div class="py-10 pt-20 px-16 flex items-center justify-center">
            <div class="scale-150 h-max w-max flex items-center justify-center py-3">
                <div class="relative w-44 h-60 flex pb-8">
                    <div class="w-5 rounded-tl-xl" style="background-color: {{Route::is('book.create') ? '#475569' : $data->cover_right_color}};" id="cover_right_color"></div>
                    <div class="w-4/5 rounded-tr-xl flex items-start justify-end pb-2" style="background-color: {{Route::is('book.create') ? '#64748B' : $data->cover_color}};"
                        id="cover_color">
                        <img src="{{Route::is('book.create') ? '' : asset('storage/'.$data->picture)}}" class="w-full h-full rounded-tr-lg {{Route::is('book.create') ? 'hidden' : ''}}" alt="" id="cover_image">
                    </div>
                    <div class="absolute bottom-5 right-[0.95rem] left-0 rounded-lg h-6" style="background-color: {{Route::is('book.create') ? '#1E293B' : $data->cover_bottom_color}};"
        ...
        <form action="{{ Route::is('book.create') ? route('book.store') : route('book.update', $data->id) }}" method="POST"
